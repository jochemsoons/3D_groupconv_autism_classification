...
from input_3Dimage import AbideDataset
...
x = tf.placeholder(tf.float32, [32, 45, 54, 45, 1])
y_ = tf.placeholder(tf.float32, [None, nLabel])
...
for i in range(100):
    # batch = train[batch_low:batch_high]
    batch_x, batch_y = abide.get_batch(batch_low, batch_high)

    print(batch_x.shape)
    print(batch_y.shape)
    # If no batch is found reset indices and continue loop
    if len(batch_x) == 0:
      batch_low = 0
      batch_high = 32
      continue
...
print("test accuracy %g"%accuracy.eval(feed_dict={x: val[0], y_: val[1], keep_prob: 1.0}))
